...
nothing in Joy listens for one. Not `joyInstance.openWidget()` — the plan proxy
intercepts it on a gated shop and returns `undefined`.

**Widget deep links are not the obvious names.** It is `#joy-referral-program`,
not `#joy-referral`. And `_handleAutoOpen` runs once at init with no `hashchange`
listener, so writing the hash on a loaded page does nothing by itself.
